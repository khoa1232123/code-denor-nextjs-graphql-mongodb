...
import { useProductCatsQuery } from "@/gql/graphql-hooks";
...
const ProductCatList = ({ setData }: Props) => {
  const router = useRouter();

  const { productCats, loading, error, errors } = useProductCatsQuery({});

  const handleEditProductCat = (item: UpdateProductCatInput) => {
    setData({
      id: item.id,
      title: item.title,
